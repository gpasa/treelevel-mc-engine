...
DEST="$APPS/TreeLevel MC Engine.app"
...
mkdir -p "$APPS"
rm -rf "$DEST"
/usr/bin/ditto "App/build/Build/Products/Release/TreeLevel MC Engine.app" "$DEST"
...
"$LSREGISTER" -u "$PWD/App/build/Build/Products/Release/TreeLevel MC Engine.app" 2>/dev/null || true
"$LSREGISTER" -u "$PWD/build/release/TreeLevel MC Engine.app" 2>/dev/null || true
"$LSREGISTER" -f "$DEST" 2>/dev/null || true

if command -v pythia8-config >/dev/null || [ -f /opt/local/include/pythia/Pythia8/Pythia.h ]; then
  make -C Backends/pythia install >/dev/null && echo "→ module Pythia dans ~/Library/Application Support/TreeLevel MC Engine/Modules/pythia8"
fi
echo "→ $DEST  ($(stat -f "%Sm" -t "%d %b %H:%M" "$DEST/Contents/MacOS/TreeLevel MC Engine"))"
